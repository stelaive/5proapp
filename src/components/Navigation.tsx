      ...
      >
        <div className="flex flex-col space-y-4">
          {navItems.slice(0, 2).map((item) => (
              <Link key={item.key} href={item.href} onClick={closeMenu} className="text-gray-800 hover:text-[#F97316] font-semibold py-2 text-lg">{item.label}</Link>
          ))}
          ...
          <div>
            <button 
              ...
              className="w-full flex justify-between items-center text-gray-800 hover:text-[#F97316] font-semibold py-2 text-lg"
            >
            ...
            {mobileLocationMenuOpen && (
              <div className="pl-4 mt-2 flex flex-col space-y-2 border-l-2" style={{ borderColor: '#F97316' }}>
                {LOCATIONS_DATA.map((location) => (
                  <Link 
                    key={location.id} 
                    href={`/locations/${location.slug}`} 
                    onClick={closeMenu}
                    className="text-gray-600 hover:text-[#F97316] py-1 pl-2"
                  >
                    {location.name}
                  </Link>
                ))}
